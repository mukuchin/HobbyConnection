...
本アプリは共通の趣味を持ったユーザー同士で盛り上がり、新しい趣味を見つけられるようにすることで人生をより楽しくすること目指しています。

特に、記事を投稿したユーザー自身に興味を持ち、そのユーザーの別の趣味に触れることで、新たな趣味をつくるというプロセスを作りたいです。
...
| 認証           | Laravel Breeze (ユーザー認証), Laravel Sanctum (API 認証)          |
| ファイル管理   | Amazon S3                                                          |
...
-   投稿・編集時の事故・不正防止機能
    ...
    -   投稿・編集時に、ユーザーが意図しない操作をしてしまうと、投稿・編集した内容が削除されてしまったり、不正なデータが投稿・編集されてしまったりする可能性があると考えました。そこで、以下の機能を実装しました。
        -   アラート付きタイマー
...
-   ユーザーフォロー機能と記事にコメントする機能の実装
    -   「記事を投稿したユーザー自身に興味を持ち、そのユーザーの別の趣味に触れることで、新たな趣味をつくるというプロセス」をつくるために、ユーザー同士のコミュニケーションを促すと考えられる、ユーザーフォロー機能と記事にコメントする機能を実装したいと考えています。
-   CI/CD の導入
    -   CI/CD を通じて自動的なテストとデプロイを行うことで、バグの早期発見や作業の効率化が実現できるため、導入する必要があると考えています。
-   コンポーネント設計の見直し
...
-   記事検索機能
-   ユーザー検索機能
-   検索したユーザーのユーザー情報・記事を閲覧できる機能（タグをクリックすると、そのタグを設定した記事が一覧表示されるようにする）
